...
# DLL fix for Python 3.7+ on Windows
if os.name == 'nt':
    base_carla = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'carla', 'WindowsNoEditor'))
    carla_bin_path = os.path.join(base_carla, 'CarlaUE4', 'Binaries', 'Win64')
    zlib_bin_path = os.path.join(base_carla, 'Engine', 'Binaries', 'ThirdParty', 'zlib', 'Win64')
    
    if hasattr(os, 'add_dll_directory'):
        if os.path.exists(carla_bin_path):
            os.add_dll_directory(carla_bin_path)
        if os.path.exists(zlib_bin_path):
            os.add_dll_directory(zlib_bin_path)
    else:
        # For Python 3.7
        os.environ['PATH'] = carla_bin_path + os.pathsep + zlib_bin_path + os.pathsep + os.environ['PATH']
...
class CarEnv:
    ...
    def __init__(self, train_mode=True):
        self.train_mode = train_mode  # Flag to determine if in training or demo mode
        ...
        self.prev_next = 0
        self.waypoints_txt = []
        self.step_count = 0  # Debug counter

        if settings.WORKING_MODE == settings.WORKING_MODE_OPTIONS[8]:  # PRE_TRAINED_CNN
            self.model_waypoints = load_model(settings.PRE_CNN_PATH)
        else:
            self.model_waypoints = None
    ...
    def crossline_data(self, event2):
        # Ignore lane crossings during the first few steps (spawn adjustment)
        if self.step_count < 30:
            return

        # Check the type of line crossed
        lane_types = set(x.type for x in event2.crossed_lane_markings)
        
        # We allow crossing 'Broken' (dashed) lines and 'NONE'
        # We only punish crossing 'Solid', 'SolidSolid', 'SolidBroken', 'BrokenSolid' etc.
        # Ideally, we primarily want to stop if it crosses a SOLID line.
        
        for x in lane_types:
            # print(f"[DEBUG] Crossed line type: {x}") # Optional debug
            pass

        # Valid types in CARLA: NONE, Broken, Solid, SolidSolid, SolidBroken, BrokenSolid, BottsDots, Grass, Curb
        # We want to trigger ONLY for Solid variants (leaving the legal lane)
        
        fail_types = [carla.LaneMarkingType.Solid, 
    ...
    def step(self, action):
        ...
        # SALIDA UTILIZADA EL RESTO DE PROGRAMAS
            ...
            if exit_flag == 1:
                print(f"[DEBUG] EXIT_FLAG TRIGGERED! Step: {self.step_count}")
                print(f"[DEBUG] Lost waypoints. Distance to target: {d2target}")
                print(f"[DEBUG] P_locales size: {len(self.current_plan)}")
                print(f"[DEBUG] pos_array_wp: {self.pos_array_wp}")
                done = True
    ...
    def get_reward(self):
        ...
        if len(self.collision_hist) != 0:  # or (len(self.crossline_hist) != 0)
            ...
            salida = 1
            reward = -200
            print(f"[DEBUG] COLLISION DETECTED! Step: {self.step_count}. Hist len: {len(self.collision_hist)}")
            # print('Collision detected, distance to target: ', d2target)
        ...
        if len(self.crossline_hist) != 0:
            ...
            salida = 1
            reward = -200
            print(f"[DEBUG] LANE DEPARTURE (SOLID LINE)! Step: {self.step_count}")
            # print('Lane departure, distance to target: ', d2target)
            self.summary['Steps'] += 1
        ...
        if salida == 0:  # SI NO HAY CONDICION DE SLAIDA DEL PROGRAMA
            ...
            # Only stop at target during training. In demo mode, continue driving.
            if self.train_mode and self.distance_target(self.Target, location) < 15:
                done = True
                reward = 100
                self.summary['Steps'] += 1
            ...
            if self.episode_start + settings.SECONDS_PER_EPISODE < time.time():
                print(f"[DEBUG] TIMEOUT! Step: {self.step_count}. Elapsed: {time.time() - self.episode_start:.2f}s")
                # print('Timeout, distance to target: ', d2target)
                done = True
                self.summary['Steps'] += 1
                if acum <= 50:
                    reward = -200
