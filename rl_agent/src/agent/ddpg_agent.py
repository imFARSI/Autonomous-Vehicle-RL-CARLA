        ...
        for cur_episode in tqdm(range(self.max_episodes)):
            ...
            for cur_step in range(self.max_steps_episode):
                ...
                if self.replay_buffer.size() > self.warmup_steps:
                    ...
                    critic_loss, predicted_q_value, _ = self.critic.train(state_batch, action_batch, y_i)
                ...
                if terminal or cur_step == self.max_steps_episode-1:
                    ...
                    train_episode_summary.value.add(simple_value=episode_critic_loss/float(cur_step), tag="train/mean_critic_loss")
                    train_episode_summary.value.add(simple_value=collision_count, tag="train/collision_count")
                    train_episode_summary.value.add(simple_value=float(cur_step), tag="train/episode_length")
